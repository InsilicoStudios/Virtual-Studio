...
public class A_PaintBrush : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (controller != null && !ray.busy)
        {
            ...
            //else if (paintPallete.PressureSensitivity)
            //{
            //    if (controller != null)
            //    {
            //        if (controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger).x > 0.2f)
            //        {
            //            // print(controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger).ToString());

            //            Paint();
            //            isPainting = true;

            //            //test continious scale
            //            if (extrusionTarget)
            //            {
            //                extrusionTarget.localScale = new Vector3(SizeX, SizeY, SizeZ);
            //                extrusionTarget.localScale = new Vector3(SizeX * controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger).x * 2, SizeY * controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger).x * 2, SizeZ * controller.GetAxis(Valve.VR.EVRButtonId.k_EButton_SteamVR_Trigger).x * 2);
            //            }
    ...
    void Paint()
    {
        if (!paintOn && !ray.busy)
        {
            ...
            //assign size
            // curPaint.transform.localScale = new Vector3(SizeX, SizeY, SizeZ);(this now has been moved in the Update for doing scaling while painting)
